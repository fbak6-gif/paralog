...
// person 0, 1 = individuals | person 2 = household shared
...
function Avatar({name,color,size=32,ring}){
  return <div style={{width:size,height:size,borderRadius:"50%",background:`${color}1a`,
    ...
    fontSize:size*.38,fontWeight:800,color,flexShrink:0,boxShadow:ring?`0 0 12px ${color}55`:"none",transition:"all .2s"}}>
    {(name||"?")[0].toUpperCase()}
  </div>;
}
...
export default function App(){
  ...
  useEffect(()=>{
    async function loadAll(){
      ...
      try{
        ...
        if(setData){
          const l=setData.find(r=>r.key==="lang");
          const b=setData.find(r=>r.key==="budgets");
          const n=setData.find(r=>r.key==="names");
  ...
  const handleSetBudgets = async fn=>{
    setBudgets(prev=>{ const next=typeof fn==="function"?fn(prev):fn; saveSetting("budgets",next); return next; });
  };
  ...
  const todaySpend  = mTxs.filter(t=>t.date===today&&t.type==="expense"&&t.category!=="transfer").reduce((s,t)=>s+t.amt,0);
  const now2        = new Date();
  ...
  const weeklyData = useMemo(()=>{
    ...
    for(let w=7;w>=0;w--){
      ...
      const es=endD.toISOString().split("T")[0];
      const ss=startD.toISOString().split("T")[0];
  ...
  async function submit(){
    ...
    try{
      if(form.mode==="transfer"){
        ...
        if(!error){ setTxs(p=>[txIn,txOut,...p]); setForm(false); setF(IF); pop(tr?"Transfer eklendi ✓":"Transfer added ✓"); }
        else pop("Error","err");
        setSyncing(false); return;
      }
      const isIncome = form.mode==="income";
      const isHouse  = form.mode==="household";
      const cat      = isIncome?form.incomeCategory:form.category;
      const person   = isHouse?2:form.person;
      const type     = isIncome?"income":"expense";
      const txId     = Date.now();
      const tx       = {id:txId,type,category:cat,amt:a,note:form.note,date:form.date,person};
      if(form.isRecurring&&!isIncome){
        const recId=txId+1;
        const r={id:recId,category:cat,amt:a,note:form.note||cl(CATS.find(c=>c.id===cat)),person};
        const {error:re}=await supabase.from("recurring").insert([r]);
        if(!re){ setRec(p=>[...p,r]); tx.rid=recId; }
      }
      const {error:te}=await supabase.from("transactions").insert([tx]);
      if(!te){
        setTxs(p=>[tx,...p]); setForm(false); setF(IF);
        pop(form.isRecurring?(tr?"Sabit eklendi ✓":"Recurring added ✓"):isIncome?(tr?"Gelir eklendi ✓":"Income added ✓"):isHouse?(tr?"Hane gideri ✓":"Household expense ✓"):(tr?"Harcama eklendi ✓":"Expense added ✓"));
      } else pop("Error","err");
        ...
        <div style={{flex:1,padding:"28px 24px",overflowY:"auto",minWidth:0}}>
                    ...
                    <div style={{height:240}}>
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={[...allMos].sort().map(mk=>{
                          ...
                          catTotals.slice(0,5).forEach(cat=>{
                            row[cat.label]=+m.filter(t=>t.category===cat.id&&t.type==="expense").reduce((s,t)=>s+t.amt,0).toFixed(2);
                          });
                          return row;
                          ...
                          {catTotals.slice(0,5).map((cat,i)=>(
                            <Line key={i} type="monotone" dataKey={cat.label} stroke={cat.color} strokeWidth={2} dot={{r:3,fill:cat.color}} activeDot={{r:5}}/>
                          ))}
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
          ...
          {view==="history"&&(
            <div>
              <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <div style={{display:"flex",background:"rgba(255,255,255,0.04)",border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
                  {[{v:"date",l:`📅 ${tr?"Tarih":"Date"}`},{v:"added",l:`🕐 ${tr?"Son Eklenen":"Recent"}`}].map(s=>(
                    <button key={s.v} onClick={()=>setHistSort(s.v)} style={{padding:"7px 12px",border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:histSort===s.v?`${C.cyan}25`:"transparent",color:histSort===s.v?C.cyan:C.sub,whiteSpace:"nowrap"}}>
                      {s.l}
                    </button>
                  ))}
                </div>
              ...
              {sortedD.length===0||histTxs.length===0
                ?<Card style={{padding:"48px 24px",textAlign:"center"}}><div style={{fontSize:40,marginBottom:12}}>📭</div><div style={{fontWeight:600,color:C.sub}}>{tr?"İşlem yok":"No transactions"}</div></Card>
                :sortedD.map(date=>(
                  <div key={date} style={{marginBottom:16}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      {grouped[date].map(tx=>{
                        ...
                        const cat=isTransfer?{icon:"↔️",color:C.yellow,glow:"255,214,0",label:"Transfer"}:(CATS.find(c=>c.id===tx.category)||CATS[0]);
                        const pi=tx.person??0;
                        return (
                          <Card key={tx.id} color={cat.color} glow={cat.glow||"255,214,0"} style={{padding:"14px 16px",display:"flex",gap:12,alignItems:"center"}}>
                            ...
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                                <div style={{fontWeight:700,fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tx.note||(isTransfer?"Transfer":cl(cat))}</div>
                                {tx.rid&&<Badge color={C.cyan}>🔁</Badge>}
                                {isTransfer&&<Badge color={C.yellow}>↔️</Badge>}
                              </div>
                              <div style={{display:"flex",alignItems:"center",gap:6}}>
                                <div style={{width:6,height:6,borderRadius:"50%",background:PA[Math.min(pi,2)]}}/>
                                <span style={{fontSize:11,color:PA[Math.min(pi,2)],fontWeight:600}}>{pi===2?"🏠":pn(pi)}</span>
                                {!isTransfer&&<span style={{fontSize:11,color:C.sub}}>· {cl(cat)}</span>}
                              </div>
                            </div>
                            <div style={{textAlign:"right",flexShrink:0}}>
                              <div style={{fontWeight:800,fontSize:16,color:tx.type==="income"?C.green:C.pink}}>{tx.type==="income"?"+":"-"}{fmt(tx.amt)}</div>
                              <button onClick={()=>deleteTx(tx)} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,fontSize:13,marginTop:3}} onMouseEnter={e=>e.currentTarget.style.color=C.pink} onMouseLeave={e=>e.currentTarget.style.color=C.muted}>✕</button>
                            </div>
                          </Card>
                        );
                      })}
          ...
          <div style={{background:"rgba(6,9,20,0.98)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:24,padding:32,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto",backdropFilter:"blur(30px)",boxShadow:`0 0 80px ${C.cyan}15`}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:22}}>
              {[
                ...
                <button key={o.m} onClick={()=>setF(f=>({...f,mode:o.m}))} style={{padding:"12px 6px",borderRadius:13,border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,fontWeight:700,fontSize:12,background:form.mode===o.m?`${o.color}20`:"rgba(255,255,255,0.04)",color:form.mode===o.m?o.color:C.sub,boxShadow:form.mode===o.m?`inset 0 0 0 1.5px ${o.color}55,0 0 16px ${o.color}20`:"inset 0 0 0 1px rgba(255,255,255,0.08)",transition:"all .2s"}}>
            ...
            {form.mode==="transfer"&&(
              <>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Gönderen":"From"}</label>
                  <div style={{display:"flex",gap:8}}>
                    {[0,1].map(i=><button key={i} onClick={()=>setF(f=>({...f,transferFrom:i}))} style={{flex:1,padding:"12px",borderRadius:13,cursor:"pointer",border:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontWeight:700,fontSize:13,background:form.transferFrom===i?`${PA[i]}20`:"rgba(255,255,255,0.04)",color:form.transferFrom===i?PA[i]:C.sub,boxShadow:form.transferFrom===i?`inset 0 0 0 1.5px ${PA[i]}55`:"inset 0 0 0 1px rgba(255,255,255,0.08)",transition:"all .2s"}}><Avatar name={pn(i)} color={PA[i]} size={26} ring={form.transferFrom===i}/>{pn(i)}</button>)}
                  </div>
                </div>
                <div style={{textAlign:"center",fontSize:20,color:C.yellow,marginBottom:14}}>↓</div>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Alan":"To"}</label>
                  <div style={{display:"flex",gap:8}}>
                    {[0,1].map(i=><button key={i} onClick={()=>setF(f=>({...f,transferTo:i}))} style={{flex:1,padding:"12px",borderRadius:13,cursor:"pointer",border:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontWeight:700,fontSize:13,background:form.transferTo===i?`${PA[i]}20`:"rgba(255,255,255,0.04)",color:form.transferTo===i?PA[i]:C.sub,boxShadow:form.transferTo===i?`inset 0 0 0 1.5px ${PA[i]}55`:"inset 0 0 0 1px rgba(255,255,255,0.08)",transition:"all .2s"}}><Avatar name={pn(i)} color={PA[i]} size={26} ring={form.transferTo===i}/>{pn(i)}</button>)}
                  </div>
                </div>
                <div style={{marginBottom:18}}>
                ...
                <div style={{background:`${C.yellow}0d`,border:`1px solid ${C.yellow}30`,borderRadius:14,padding:"12px 16px",marginBottom:20}}>
                  <div style={{color:C.yellow,fontWeight:700,marginBottom:4,fontSize:13}}>↔️ {tr?"Özet":"Preview"}</div>
                  <div style={{color:C.sub,fontSize:13}}><span style={{color:PA[form.transferFrom],fontWeight:700}}>{pn(form.transferFrom)}</span> → <span style={{color:PA[form.transferTo],fontWeight:700}}>{pn(form.transferTo)}</span> · <span style={{color:C.yellow,fontWeight:800}}>{form.amount?fmt(parseFloat(form.amount)||0):`${CAD}0.00`}</span></div>
                  <div style={{fontSize:11,color:C.muted,marginTop:4}}>{tr?"Gönderenden düşülür, alana eklenir":"Deducted from sender, added to receiver"}</div>
                </div>
              </>
            )}
              ...
              <>
                ...
                <div style={{marginBottom:18}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {EXP_CATS.map(cat=><button key={cat.id} onClick={()=>setF(f=>({...f,category:cat.id}))} style={{padding:"7px 11px",borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:form.category===cat.id?700:400,display:"flex",alignItems:"center",gap:5,border:"none",background:form.category===cat.id?`${cat.color}20`:"rgba(255,255,255,0.04)",color:form.category===cat.id?cat.color:C.sub,boxShadow:form.category===cat.id?`inset 0 0 0 1px ${cat.color}55`:"inset 0 0 0 1px rgba(255,255,255,0.08)",transition:"all .15s"}}><span>{cat.icon}</span><span>{cl(cat)}</span></button>)}
                  </div>
                </div>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Tutar":"Amount"} (CAD)</label>
                  <div style={{position:"relative"}}>
                    <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:C.sub,fontSize:17,fontWeight:700}}>{CAD}</span>
                    <input type="number" min="0" step="0.01" value={form.amount} onChange={e=>setF(f=>({...f,amount:e.target.value}))} placeholder="0.00" style={{...inp,paddingLeft:50,fontSize:22,fontWeight:800,color:C.pink}}/>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
                  <div><label style={lbl}>{tr?"Not":"Note"}</label><input type="text" value={form.note} onChange={e=>setF(f=>({...f,note:e.target.value}))} placeholder={tr?"Opsiyonel...":"Optional..."} style={inp}/></div>
                ...
                <div onClick={()=>setF(f=>({...f,isRecurring:!f.isRecurring}))} style={{display:"flex",alignItems:"center",gap:14,marginBottom:24,padding:"14px 16px",borderRadius:14,cursor:"pointer",background:form.isRecurring?`${C.cyan}0e`:"rgba(255,255,255,0.03)",border:`1px solid ${form.isRecurring?C.cyan+"44":"rgba(255,255,255,0.08)"}`,transition:"all .2s"}}>
                  <div style={{width:24,height:24,borderRadius:7,border:`2px solid ${form.isRecurring?C.cyan:"rgba(255,255,255,0.2)"}`,background:form.isRecurring?`linear-gradient(135deg,${C.cyan},${C.purple})`:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .2s"}}>{form.isRecurring&&<span style={{color:"#fff",fontSize:14,fontWeight:900}}>✓</span>}</div>
                  <div><div style={{fontWeight:700,fontSize:13,color:form.isRecurring?C.cyan:C.text}}>🔁 {tr?"Her ay tekrarlayan":"Recurring monthly"}</div><div style={{fontSize:11,color:C.sub,marginTop:2}}>{tr?"Her ay otomatik eklenir":"Auto-added every month"}</div></div>
                </div>
              </>
            )}
            ...
            {form.mode==="income"&&(
              <>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Kişi":"Person"}</label>
                  <div style={{display:"flex",gap:10}}>
                    {[0,1].map(i=><button key={i} onClick={()=>setF(f=>({...f,person:i}))} style={{flex:1,padding:"13px 16px",borderRadius:14,cursor:"pointer",border:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontWeight:700,fontSize:14,background:form.person===i?`${PA[i]}20`:"rgba(255,255,255,0.04)",color:form.person===i?PA[i]:C.sub,boxShadow:form.person===i?`inset 0 0 0 1px ${PA[i]}55`:"inset 0 0 0 1px rgba(255,255,255,0.08)",transition:"all .2s"}}><Avatar name={pn(i)} color={PA[i]} size={28} ring={form.person===i}/>{pn(i)}</button>)}
                  </div>
                </div>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Gelir Kaynağı":"Income Source"}</label>
                  <div style={{display:"flex",gap:8}}>
                    {INC_CATS.map(cat=><button key={cat.id} onClick={()=>setF(f=>({...f,incomeCategory:cat.id}))} style={{flex:1,padding:"12px 8px",borderRadius:12,cursor:"pointer",fontSize:13,fontWeight:form.incomeCategory===cat.id?700:400,display:"flex",alignItems:"center",justifyContent:"center",gap:7,border:"none",background:form.incomeCategory===cat.id?`${cat.color}20`:"rgba(255,255,255,0.04)",color:form.incomeCategory===cat.id?cat.color:C.sub,boxShadow:form.incomeCategory===cat.id?`inset 0 0 0 1px ${cat.color}55`:"inset 0 0 0 1px rgba(255,255,255,0.08)"}}><span style={{fontSize:18}}>{cat.icon}</span><span>{cl(cat)}</span></button>)}
                  </div>
                </div>
                <div style={{marginBottom:18}}>
                  <label style={lbl}>{tr?"Tutar":"Amount"} (CAD)</label>
